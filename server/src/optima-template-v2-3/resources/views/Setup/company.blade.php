...
<div class="row">
                ...
                <td>
                  <center>
                    ...
                    <a onclick="grid_edit({{$pmKey}},{{$primaryKey}})"><i class="icon-pencil text-success pr-1"></i></a>
                        {{-- compstatus = 1 sebagai admin utama jadi kalau di admin umum delete dan management tidak tersedia --}}
                    @if($compstatus == 1) 
                    <a onclick="management({{$pmKey}},{{$primaryKey}})"><i class="icon-insert-template text-secondary pr-1"></i></a>
  ...
  <!-- Modal with h2 -->
  <div id="management_modal" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h2 class="modal-title">Management Company</h2>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        <form id="mng">
          <div class="modal-body">
            <input type="hidden" id="form_mng" name="form_mng" value="{{json_encode($mng_form)}}">
            {{-- @csrf --}}
            <div class="row">
              @foreach($mng_form as $dataform)
              @if($dataform['type']=='text')
              <div class="col-sm-{{$dataform['col'] ?? 12}}">
                <div class="form-group">
                  <label class="form-label"><b>{{$dataform['label']}}</b></label>
              ...
              <div class="col-sm-{{$dataform['col'] ?? 12}}">
                <div class="form-group">
                  <label class="form-label"><b>{{$dataform['label']}}</b></label>
                  <input type="hidden" id="{{$dataform['field']}}_id" name="{{$dataform['field']}}_id" required>
                  <input type="text" class="form-control" id="{{$dataform['field']}}" name="{{$dataform['field']}}"
        ...
        <div class="modal-footer">
          <button type="button" id="mng" class="btn bg-primary" onclick="save_management()"><i class="icon-sync">
            </i> Sync Setup </button>
        </div>
      </div>
    </div>
...
<script type="text/javascript">
  function management(id,primaryKey){
    ...
    for (var i = 0; i < dataobj.length; i++) {
      ...
      if (eval(a) == id) {
        ...
        var field = document.getElementById('form_mng').value;
        var fieldobj = JSON.parse(field);

        // Validasi Jika Sudah Di Sync Setup
        if(dataobj[i].compStatusMng == 1){
          alertify.warning('Setup Company Sudah Di Lakukan');
          setTimeout(function() {
            window.open("{{$mainroute}}", "_self");
          }, 1000);
        }else{
          $('#management_modal').modal("show");
        }
        ...
        for (var j = 0; j < fieldobj.length; j++) {
          ...
          if (fieldobj[j].type != 'password') {
            if(fieldobj[j].field == 'compMngNama'){
              $('#'+fieldobj[j].field+'_id').val(dataobj[i].compId);
              $('#'+fieldobj[j].field).val(dataobj[i].compNama);
            }else if (fieldobj[j].field == 'compMngKategori') {
              $('#'+fieldobj[j].field).val(dataobj[i].compKategori).find(':selected').trigger('change');
            } else if (fieldobj[j].field == 'compMngDb'){
              $('#'+fieldobj[j].field).val(dataobj[i].compDb);
            }
          }
        }
      }
...
function save_management(){
  $.ajax({
          ...
          url: "/save-management",
          data: ({
            compId: $('#compMngNama_id').val(),
            compNama: $('#compMngNama').val(),
            compKategori: $('#compMngKategori').val(),
            compDb: $('#compMngDb').val(),
          }),
          dataType: "json",
          async: false,
          success: function(data) {
            alertify.success('Setup Berhasil');
            setTimeout(function() {
              window.open("{{$mainroute}}", "_self");
            }, 500);
          },
    ...
    $('#form').on('submit', function(event) {
      ...
      $.ajax({
        ...
        success: function(data) {
          // console.log(data);
          if (data.status == 401) {
            alertify.error('Form Wajib Harus diisi / Size Foto Melebihi Batas Max!');
            setTimeout(function() {
              window.open("{{$mainroute}}", "_self");
            }, 500);
